...
class SpeciesNameAdmin(admin.ModelAdmin):
    ...
    @admin.display(description="Species", ordering="species__sciname")
...
class HasGbifusagekeyFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(gbifusagekey__isnull=False)
        if self.value() == "n":
            return queryset.filter(gbifusagekey__isnull=True)
...
class PortraitIsSynonymFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(species__accepted_species__isnull=False)
        if self.value() == "n":
            return queryset.filter(species__accepted_species__isnull=True)
...
class IsSynonymFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(accepted_species__isnull=False)
        if self.value() == "n":
            return queryset.filter(accepted_species__isnull=True)
...
class HasSynonymsFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(species__isnull=False)
        if self.value() == "n":
...
class HasAvatarFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(avatar_new__isnull=False)
        if self.value() == "n":
            return queryset.filter(avatar_new__isnull=True)
...
class HasFemaleAvatarFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(female_avatar_new__isnull=False)
        if self.value() == "n":
            return queryset.filter(female_avatar_new__isnull=True)
...
class HasPrimaryName(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "g":
            return queryset.filter(gername__isnull=False)
        if self.value() == "e":
            return queryset.filter(engname__isnull=False)
        if self.value() == "b":
            return queryset.filter(engname__isnull=False, gername__isnull=False)
        if self.value() == "i":
            return queryset.filter(
                models.Q(engname__isnull=False) | models.Q(gername__isnull=False)
            )
        if self.value() == "n":
            return queryset.filter(engname__isnull=True, gername__isnull=True)
...
class HasAdditionalNames(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(speciesname__isnull=False)
        if self.value() == "n":
            return queryset.filter(speciesname__isnull=True)
...
class HasPlantnetPowoidFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(plantnetpowoid__isnull=False)
        if self.value() == "n":
            return queryset.filter(plantnetpowoid__isnull=True)
...
class HasPlantnetPowoidMappingFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(plantnetpowoidmapping__isnull=False)
        if self.value() == "n":
            return queryset.filter(plantnetpowoidmapping__isnull=True)
...
class HasNbclassidFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(nbclassid__isnull=False)
        if self.value() == "n":
            return queryset.filter(nbclassid__isnull=True)
...
class HasBirdnetIdFilter(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(birdnetid__isnull=False)
        if self.value() == "n":
            return queryset.filter(birdnetid__isnull=True)
...
class SpeciesAdmin(admin.ModelAdmin):
    ...
    fields = [
        ...
        "autoid",
        "nbclassid",
        ("red_list_germany", "iucncategory"),
        ("activity_start_month", "activity_end_month"),
        ("activity_start_hour", "activity_end_hour"),
        "avatar_new",
        "female_avatar_new",
        "gbifusagekey",
        "accepted_species",
    ...
    def get_queryset(self, request):
        ...
        if request.resolver_match.view_name.endswith("changelist"):
            return qs.select_related("avatar_new", "group").prefetch_related(
                "portrait_set"
            )
        return qs
    ...
    def search(self, obj):
        plantnet_url = (
            f"https://identify.plantnet.org/k-world-flora/species?search={obj.sciname}"
        )
    ...
    @admin.display(description="Powo ID (Plantnet)")
    ...
    def accepted(self, obj):
        if obj.accepted_species is None:
            ...
            url = reverse(
                "admin:species_species_change", args=(obj.accepted_species.id,)
            )
            return format_html(
    ...
    def portrait(self, obj):
        if obj.group.nature is None or obj.accepted_species:
            ...
            for lang in ["de", "en"]:
                portrait = [
                    portrait
                    for portrait in obj.portrait_set.all()
                    if portrait.language == lang
                ]
                if portrait:
                    url = reverse(
                        f"admin:species_{obj.group.nature}portrait_change",
                        args=([portrait[0].id]),
                    )
...
def copy_portrait_to_eng(modeladmin, request, queryset):
    ...
    if model.objects.filter(
        Q(species_id__in=queryset.values_list("species__id", flat=True))
        & Q(language="en")
    ).exists():
        messages.error(
            request,
    ...
    with transaction.atomic():
        for portrait in queryset.select_related("species__accepted_species"):
            ...
            if accepted_species:
                ...
                if (
                    synonym_species.female_avatar_new
                    and not accepted_species.female_avatar_new
                ):
                    accepted_species.female_avatar_new = (
                        synonym_species.female_avatar_new
                    )
                    accepted_species.save()
...
class FaunaportraitAudioFileAdmin(admin.ModelAdmin):
    ...
    fields = ["audio_file", "species", "owner", "owner_link", "source", "license"]
...
class HasSpecies(YesNoFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "y":
            return queryset.filter(
                Q(avatar_new_species__isnull=False)
                | Q(female_avatar_new_species__isnull=False)
            )
        if self.value() == "n":
            return queryset.filter(
                Q(avatar_new_species__isnull=True)
                & Q(female_avatar_new_species__isnull=True)
            )
...
class PlantnetPowoidMappingAdmin(admin.ModelAdmin):
    ...
    def species(self, obj):
        link = reverse(
            "admin:species_species_change", args=[obj.species_plantnetpowoid.id]
        )
        return format_html(
            '<a href="{}">{} ({})</a>',
            link,
...
class ImageFileAdmin(admin.ModelAdmin):
    ...
    def add_crop_link(self, obj):
        ...
        if maybe_imagecrop:
            ...
            url = reverse(
                "admin:species_imagecrop_change", args=([maybe_imagecrop[0].id])
            )
            return format_html('<a href="{}" class="changelink"></a>', url)
        else:
            url = reverse("admin:species_imagecrop_add") + f"?imagefile={obj.id}"
...
class LeichtPortraitAdmin(admin.ModelAdmin):
    ...
    @admin.display(description="Goodtoknow Image")
