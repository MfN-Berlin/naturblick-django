...
class SpeciesSerializer(serializers.ModelSerializer):
    ...
    avatar_owner = serializers.CharField(
        source="avatar_new.owner", read_only=True)
    avatar_license = serializers.CharField(
        source="avatar_new.license", read_only=True)
    avatar_source = serializers.CharField(
        source="avatar_new.source", read_only=True)
    audio_filename = serializers.SerializerMethodField(
        method_name="create_audio_filename")
...
class SimilarSpeciesSerilizer(serializers.ModelSerializer):
    ...
    similar_species_id = serializers.CharField(
        source="species.id", read_only=True)
    avatar_url = AvatarCropUrlField(
        source='species.avatar_new')
    localname = SimilarSpeciesLocalnameField(source='*')
    speciesid = serializers.CharField(
        source="species.speciesid", read_only=True)
    group = serializers.CharField(source='group.name', read_only=True)
...
class DescMetaSerializer(serializers.Serializer):
    image_orientation = serializers.CharField(
        source="descmeta.image_orientation", read_only=True)
    display_ratio = serializers.CharField(
        source="descmeta.display_ratio", read_only=True)
    grid_ratio = serializers.CharField(
        source="descmeta.grid_ratio", read_only=True)
    focus_point_vertical = serializers.FloatField(
        source="descmeta.focus_point_vertical", read_only=True)
    focus_point_horizontal = serializers.FloatField(
        source="descmeta.focus_point_horizontal", read_only=True)
    text = serializers.CharField(source="descmeta.text", read_only=True)
    image_orig = serializers.URLField(
        source="descmeta.image_file.image.url", read_only=True)
    image_orig_width = serializers.IntegerField(
        source="descmeta.image_file.width", read_only=True)
    image_orig_height = serializers.IntegerField(
        source="descmeta.image_file.height", read_only=True)
    image_small = serializers.URLField(
        source="descmeta.image_file.image_small.url", read_only=True)
    ...
    owner_link = serializers.CharField(
        source="descmeta.image_file.owner_link", read_only=True)
    source = serializers.CharField(
        source="descmeta.image_file.source", read_only=True)
    license = serializers.CharField(
        source="descmeta.image_file.license", read_only=True)


class FunfactMetaSerializer(serializers.Serializer):
    image_orientation = serializers.CharField(
        source="funfactmeta.image_orientation", read_only=True)
    display_ratio = serializers.CharField(
        source="funfactmeta.display_ratio", read_only=True)
    grid_ratio = serializers.CharField(
        source="funfactmeta.grid_ratio", read_only=True)
    focus_point_vertical = serializers.FloatField(
        source="funfactmeta.focus_point_vertical", read_only=True)
    focus_point_horizontal = serializers.FloatField(
        source="funfactmeta.focus_point_horizontal", read_only=True)
    text = serializers.CharField(source="funfactmeta.text", read_only=True)
    image_orig = serializers.URLField(
        source="funfactmeta.image_file.image.url", read_only=True)
    image_orig_width = serializers.IntegerField(
        source="funfactmeta.image_file.width", read_only=True)
    image_small = serializers.URLField(
        source="funfactmeta.image_file.image_small.url", read_only=True)
    image_medium = serializers.URLField(
        source="funfactmeta.image_file.image_medium.url", read_only=True)
    image_large = serializers.URLField(
        source="funfactmeta.image_file.image_large.url", read_only=True)
    owner = serializers.CharField(
        source="funfactmeta.image_file.owner", read_only=True)
    owner_link = serializers.CharField(
        source="funfactmeta.image_file.owner_link", read_only=True)
    source = serializers.CharField(
        source="funfactmeta.image_file.source", read_only=True)
    license = serializers.CharField(
        source="funfactmeta.image_file.license", read_only=True)


class InthecityMetaSerializer(serializers.Serializer):
    image_orientation = serializers.CharField(
        source="inthecitymeta.image_orientation", read_only=True)
    display_ratio = serializers.CharField(
        source="inthecitymeta.display_ratio", read_only=True)
    grid_ratio = serializers.CharField(
        source="inthecitymeta.grid_ratio", read_only=True)
    focus_point_vertical = serializers.FloatField(
        source="inthecitymeta.focus_point_vertical", read_only=True)
    focus_point_horizontal = serializers.FloatField(
        source="inthecitymeta.focus_point_horizontal", read_only=True)
    text = serializers.CharField(source="inthecitymeta.text", read_only=True)
    image_orig = serializers.URLField(
        source="inthecitymeta.image_file.image.url", read_only=True)
    image_orig_width = serializers.IntegerField(
        source="inthecitymeta.image_file.width", read_only=True)
    image_small = serializers.URLField(
        source="inthecitymeta.image_file.image_small.url", read_only=True)
    image_medium = serializers.URLField(
        source="inthecitymeta.image_file.image_medium.url", read_only=True)
    image_large = serializers.URLField(
        source="inthecitymeta.image_file.image_large.url", read_only=True)
    owner = serializers.CharField(
        source="inthecitymeta.image_file.owner", read_only=True)
    owner_link = serializers.CharField(
        source="inthecitymeta.image_file.owner_link", read_only=True)
    source = serializers.CharField(
        source="inthecitymeta.image_file.source", read_only=True)
    license = serializers.CharField(
        source="inthecitymeta.image_file.license", read_only=True)


class PortraitSerializer(serializers.ModelSerializer):
    similar_species = SimilarSpeciesSerilizer(
        many=True, read_only=True, source='similarspecies_set')
    goodtoknow = GoodtoknowSerilizer(
        many=True, read_only=True, source='goodtoknow_set')
    unambigousfeature = UnambigousFeatureSerilizer(
        many=True, read_only=True, source='unambigousfeature_set')
...
class FaunaPortraitSerializer(PortraitSerializer):
    audio_license = serializers.CharField(
        source="faunaportrait_audio_file.license", read_only=True)
    audio_filename = serializers.SerializerMethodField(
        method_name="create_audio_filename")
